...
func main() {
	// Read the YAML file
	yamlFile, err := ioutil.ReadFile("../azure-pucar/input.yaml")
	if err != nil {
	...
	replaceInFile("../azure-pucar/variables.tf", data, false)
	fmt.Println("variables.tf file updated successfully!")

	replaceInFile("../azure-pucar/remote-state/variables.tf", data, false)
	fmt.Println("remote-state/variables.tf file updated successfully!")

	replaceInFile("../azure-pucar/main.tf", data, false)
